...
POWER SAFETY -- windows-gfx1151 only: this host is a Strix Halo APU whose CPU cores and iGPU share one power/thermal rail. Unattended `-j16` compiles layered with GPU test load tripped a platform power cutoff and hard-rebooted the machine twice (2026-06-03; not a BSOD -- abrupt power loss, no dump, no WHEA). On this host CAP build parallelism at `-j6` (the `-j16`/`-j$(nproc)` values in project notes are tuned for the 128-core Linux Jenkins hosts -- do NOT use them here) and NEVER overlap a heavy compile with a GPU test run (serialize phases). Other hosts (gfx90a/gfx1100, 128 cores) keep `-j16`. Full detail in local memory [[gfx1151-host-power-reboots]].
