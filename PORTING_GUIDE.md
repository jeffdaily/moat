...
- Texture/surface coherency (layered arrays). HIP runtime bug, confirmed on gfx90a/CDNA2 (ROCm 7.x): a `tex2DLayered` fetch does not observe a `surf2DLayeredwrite` made to the same `cudaArrayLayered` array by a PRIOR kernel launch on the same stream (it returns stale/pre-write data), even though a `surf2DLayeredread` of that location returns the fresh value. A standalone repro isolates it to the layered path: a non-layered 2D array on the same code path returns fresh data, an explicit `hipDeviceSynchronize` between launches does not help, and recreating the texture object after the write does not help -- so it is neither a write-flush nor a stale-descriptor problem, and not a hardware limit; the layered-image texture cache is simply not invalidated at the launch boundary. CUDA and HIP both document cross-launch texture/surface coherency, so in-spec separate-launch code is mishandled. Fix: read the consumer via the surface (`surf2DLayeredread`), which is coherent; recreating the texture does NOT work for layered arrays. (RDNA-vs-CDNA unverified; the real axis is layered-vs-2D. popsift make_dog read an all-zero DoG.)
...
- A CPU-only docker build (image `rocm/dev-ubuntu-24.04` or similar) proves the code compiles and links under ROCm. It cannot observe any Fault-class bug above, since no GPU runs, so it is never a validation gate. Do NOT wire it into the fork's GitHub Actions: a yml change bumps the fork HEAD sha and forces every platform to revalidate (churn), and the run just fails and emails. Disable Actions on the fork instead; run a CPU-only docker build locally if you want a manual compile check.
...
- 2026-05-30 -- HIP runtime bug (confirmed, gfx90a/CDNA2, ROCm 7.x): tex2DLayered does not see a prior-launch surf2DLayeredwrite to the same cudaArrayLayered array (returns stale/pre-write); a non-layered 2D array on the same path is fine, hipDeviceSynchronize and recreating the texture do not help -- the layered-image texture cache is not invalidated at the launch boundary. CUDA and HIP both document cross-launch coherency, so in-spec code breaks. Fix: read the consumer via surf2DLayeredread; the axis is layered-vs-2D (RDNA unverified). Standalone repro + bug report in agent_space/popsift_texsurf_coherency -- popsift
