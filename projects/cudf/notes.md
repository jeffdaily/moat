...
Verified directly (no problems):
- Fix correctness (merge.cu:139-167): the HIP path routes materialize_merged_bitmask_kernel through the converged `while (tile_any_32(tid < n))` loop + `ballot_32(active && source_bit_is_valid)` 32-lane tile shim, leader test `0 == threadIdx.x % cudf::detail::warp_size` where cudf::detail::warp_size is the constexpr 32 (cuda.cuh:36), NOT the HIP builtin warpSize (64). Confirmed the include chain resolves all three (merge.cu includes cuda.cuh:24; cuda.cuh:22 includes warp_primitives.cuh). Launch is BLOCK_SIZE=256 (merge.cu:202), and tid=global_thread_id()=threadIdx.x+blockIdx.x*256, stride=grid_stride()=256*gridDim.x -- both = 0 mod 32, so lane k always has tid%32==k for every iteration; its tile ballot bit lands at bit k = intra_word_index(tid) and the tile leader (tid%32==0) writes word_index(tid)=tid/32. Each 32-bit validity word is written by its own 32-lane-tile leader, so BOTH words of a 64-thread span (rows 0-31 AND 32-63) are written -- correct on wave64 and wave32. The `active &&` on the leader guard is a harmless safety net (lane 0 is the lowest tid in its word, so it is in-range whenever any lane in the word is). OOB lanes short-circuit `active && source_bit_is_valid` (the is_valid_nocheck deref is inside `if (active)`), so no OOB neighbor read. Arch-unified single USE_HIP path; no gfx-specific / wave==64 branching (the only "wave64" tokens in the delta are a CMake comment and the test name).
- CUDA path byte-for-byte unchanged: the `#else` block (merge.cu:168-192) is identical to upstream tag v25.08.00 (6cea374) merge.cu -- carried active_mask __ballot_sync(0xffffffffu) grid-stride dance, `threadIdx.x % warpSize` leader (warpSize=32 on nvcc). Diffed against `git show 6cea374:...`.
- Build-system: merge/merge.cu added to cpp/cmake/hip/cudf_hip_sources.cmake CUDF_HIP_CORE_SOURCES (204 -> 205 sources). Without it merge does not compile on HIP (the raw 64-lane ballot braced-init narrows 64->32, a hard error), which is why merge was originally omitted. Correct.
- Test adequacy + decisive A/B (the bug is silent, so the test had to be proven to catch it): MergeNullableBitmaskWave64 (core_smoke_test.cu:813) merges two sorted nullable INT32 columns keyed on a NON-null disjoint key (left=even, right=odd, n_total=260>64) so merged row k = left[k/2] (k even) / right[k/2] (k odd) is fully predictable independent of cudf's null tie-break; it asserts the output null mask word-by-word per row PLUS an explicit word-1 (rows 32-63) check. I re-derived the porter's uncommitted buggy variant (64-lane `__ballot` truncated to bitmask_type + lane-0-of-64-wavefront write, leader `threadIdx.x % warpSize`), rebuilt merge.cu + relinked (single build), and ran it: FAILED with 38 mismatches concentrated in the upper words, including the exact word-1 rows 32-37,38,44,45,50,53,56,61,62 and word-7 row 254 (the dropped second 32-bit word of each 64-wide wavefront). Restored the fixed version, rebuilt: PASS. The test is a real, decisive gate.
- GPU run (gfx90a / MI250X, GCD 3): full MOAT_CORE_SMOKE_TEST 19/19 PASS, deterministic over two full runs (8.8 s) plus an isolated merge-test run; MergeNullableBitmaskWave64 OK each time.
- libcudf.so single-.so relocation fix preserved: 152 embedded code objects, ALL gfx90a (151 prior + 1 for merge.cu), zero non-gfx90a; 589,663,656 B, no -mcmodel=large, no split, --offload-compress intact.
- Commit hygiene: single curated commit, title "[ROCm] HIP build for AMD GPUs (USE_HIP): libcudf core + algorithms" (66 chars, <=72, [ROCm] prefix); body describes the merge fix accurately (the seventh per-lane site, the wave64 truncation + dropped-word mechanism, the tile_any_32/ballot_32/logical-warp_size route), mentions Claude (Anthropic), no Co-Authored-By noreply trailer, no ghstack, no em-dash, ASCII-clean delta, no AMD-internal account refs. Fork main is a clean upstream mirror (HEAD is an ordinary upstream commit, no [ROCm]); fork Actions disabled (permissions enabled:false); moat-port HEAD = c9b6024 matches local + status.json.

Build hygiene note (not a defect in the port): on arrival two concurrent `cmake --build -j32` (two `ninja -j 32`) were racing the SAME projects/cudf/build-hip dir (both compiling reductions/mean.cu.o) -- exactly the .ninja_deps-corruption hazard the porter documented. merge.cu.o mtime (04:20) was newer than libcudf.so (04:17), i.e. the on-disk .so did not yet contain the final object. I let both drain, then did a single clean `cmake --build` (relinked lib+test, exit 0) before verifying, so the GPU run reflects the committed source. No source change resulted.

Note (separate audit, not this review's scope): other raw __ballot_sync sites (json_path.cu, io/json data_casting.cu, rolling, block_utils.cuh) remain untouched in this delta and are a forward-looking wave64 audit when those TUs are brought into scope.

State: linux-gfx90a ported -> review-passed (validator next exercises it on the gfx90a GPU).
