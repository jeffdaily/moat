...
Fast broad graph-path run (full suite minus GeometryTests, NeuronPerformanceTests, and the 3 extreme long-runners): 2975 ran, 2970 PASSED, 3 SKIPPED (gtest GTEST_SKIP, unrelated), 2 FAILED. The 2 failures are the two analyzed below; everything else passes. The 2 BalanceTests.longRunning_* (20000 timesteps each) pass in graph mode; ConstructorTests.regressionTestMassiveReplicationsWithSeeds (10000 steps, external energy 1e7 -> exploding self-replicator population, ~22 TPS as it grows) has no assertions and passes on fault-free completion but is very slow (excluded from the fast broad runs; run it standalone if needed). Both paths run the same kernels; -d adds a cudaDeviceSynchronize after every kernel (so heavy tests are 10-40x slower in -d).
