...
4. Where the project uses GitHub Actions, add a CPU-only ROCm build smoketest workflow to the fork (image `rocm/dev-ubuntu-24.04`). It proves COMPILATION ONLY and can never observe a warpSize/OOB/pitch fault, so it is a contributor tripwire, never the validation gate.
